...
function BrowserPane({siteToolsEnabled=true,onAnnotate,onClose}){
  ...
  useEffect(()=>{
    ...
    const off=window.desktopApi.onBrowserState?.(next=>{
      setState(next);
      if(next.url&&!addressEditing)setUrl(next.url);
    });
  ...
  return <aside className="sidePane browserPane">
    ...
    <div className="browserToolbar">
      ...
      <form onSubmit={e=>{e.preventDefault();navigate()}}><input value={url} onFocus={()=>setAddressEditing(true)} onBlur={()=>{setAddressEditing(false);if(state.url)setUrl(state.url)}} onChange={e=>setUrl(e.target.value)} placeholder="Search or enter a URL" aria-label="Address and search"/></form>
